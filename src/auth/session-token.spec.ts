...
describe('verifyBetterAuthSessionCookie', () => {
  ...
  it('accepts the URL-encoded cookie emitted in an HTTP header', () => {
    expect(
      verifyBetterAuthSessionCookie(encodeURIComponent(signedCookie()), SECRET),
    ).toBe(TOKEN);
  });
...
describe('SessionTokenService', () => {
  ...
  it.each(['better-auth.session_token', '__Secure-better-auth.session_token'])(
    'accepts the signed %s cookie',
    (cookieName) => {
      expect(
        service.extract(request({ cookies: { [cookieName]: signedCookie() } })),
      ).toBe(TOKEN);
    },
  );
  ...
  it('accepts an explicitly raw Bearer session token', () => {
    expect(service.extract(request({ authorization: `Bearer ${TOKEN}` }))).toBe(
      TOKEN,
    );
  });
