...
function extractPeriodo(text: string): string | null {
  ...
  if (/\bDiversos\b/i.test(text) && /Parcel|PARCSN/i.test(text)) {
    const venc = text.match(
      /(?:Pagar\s+(?:este\s+documento\s+)?at[eé]|Vencimento)[^\d]*(\d{2})\/(\d{2})\/(20\d{2})/i,
    );
    if (venc) return `${venc[2]}/${venc[3]}`;
  }
  const contextual = text.match(
    /(?:Per[ií]odo de Apura[cç][aã]o|Compet[eê]ncia|Refer[eê]ncia)[^\d]*(0[1-9]|1[0-2])\/(20\d{2})/i,
  );
...
function detectTipo(text: string): TipoDocumento {
  if (/Simples Nacional/i.test(text)) {
    if (/PGFN[-\s]?SISPAR|REC\.?\s*DIVIDA\s*ATIVA/i.test(text))
      return 'DAS-PGFN';
  ...
  if (
    /DAE|documento\s+de\s+arrecada[cç][aã]o\s+estadual|ANTECIPA[CÇC][AÃA]O\s+PARCIAL/i.test(
      text,
    )
  )
    return 'DAE';
...
function extractParcelamento(
  text: string,
  tipo: TipoDocumento,
): DadosFiscais['parcelamento'] {
  if (tipo !== 'DAS-PARCSN' && tipo !== 'DAS-PGFN' && tipo !== 'PGFN-SISPAR') {
    return null;
  }
