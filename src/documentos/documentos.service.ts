...
export class DocumentosService {
  ...
  async listAdminDocuments(input: {
    ...
    // Exclude FOLHA-PAGAMENTO from general document listing (managed in RH module)
    conditions.push(ne(documentos.tipo, 'FOLHA-PAGAMENTO'));
    ...
    if (input.status === 'PENDENTE') {
      conditions.push(
        and(
          eq(documentos.status, 'PENDENTE'),
          or(isNull(documentos.vencimento), gte(documentos.vencimento, today)),
        )!,
      );
    }
    if (input.search) {
    ...
    const [countResult, rows] = await Promise.all([
      ...
      this.database.db
        .select({
          ...
          visualizado:
            sql<boolean>`EXISTS (SELECT 1 FROM visualizacoes_documentos WHERE visualizacoes_documentos.documento_id = ${documentos.id})`.as(
              'visualizado',
            ),
          primeiraVisualizacao: sql<
            string | null
          >`(SELECT MIN(visualizado_em) FROM visualizacoes_documentos WHERE visualizacoes_documentos.documento_id = ${documentos.id})`.as(
            'primeira_visualizacao',
          ),
          totalVisualizacoes:
            sql<number>`(SELECT COUNT(*) FROM visualizacoes_documentos WHERE visualizacoes_documentos.documento_id = ${documentos.id})`.as(
              'total_visualizacoes',
            ),
        })
        .from(documentos)
  ...
  async listClientDocumentsForStaff(input: {
    ...
    // Exclude FOLHA-PAGAMENTO from general document listing
    conditions.push(ne(documentos.tipo, 'FOLHA-PAGAMENTO'));
    if (input.type) conditions.push(eq(documentos.tipo, input.type));
    const periodCondition = this.documentPeriodCondition(
      input.period,
      input.periodType,
    );
    ...
    const [countResult, rows] = await Promise.all([
      ...
      this.database.db
        .select({
          ...
          installmentNumber: documentos.numeroParcelamento,
          visualizado:
            sql<boolean>`EXISTS (SELECT 1 FROM visualizacoes_documentos WHERE visualizacoes_documentos.documento_id = ${documentos.id} AND visualizacoes_documentos.user_id = ${input.userId})`.as(
              'visualizado',
            ),
        })
        .from(documentos)
  ...
  async recordDocumentView(documentId: string, userId: string) {
    await this.database.db
      ...
      .values({ documentoId: documentId, userId });
  }
  ...
  async uploadDocument(input: {
    ...
    try {
      ...
      this.logger.error('document_upload_storage_failed', error, {
        requestId: input.requestId,
        obligationId,
      });
    ...
    try {
      ...
      this.logger.error('document_upload_database_failed', error, {
        requestId: input.requestId,
        obligationId,
      });
  ...
  async notifyDocument(input: {
    ...
    try {
      ...
      this.logger.error('document_notification_provider_failed', error, {
        requestId: input.requestId,
      });
    }
